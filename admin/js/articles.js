...
function articleAdd(caller) {
    ...
    var frmData = new FormData(frm);
    if (frmData.get('title').length < 1) {
        alert('You have to add a headline');
        return;
    }
    if (frmData.get('article').length < 1) {
        alert('You have not added any content');
        return;
    }
    if (frmData.get('catId') == 0) {
        alert('Remember to chose a category');
        return;
    }
    ...
    fetch('/menuitems', { method: 'get' })
        ...
        .then(function (jsonData) {
            if (jsonData) {
                ...
                option.value = 0;
                option.textContent = "CHOSE CATEGORY";
    ...
    fetch('/menuitems', { method: 'get' })
        ...
        .then(function (jsonData) {
            if (jsonData) {
                ...
                title.type = "text";
                title.name = "title";
                title.placeholder = "CONTENTS HEADLINER...";
